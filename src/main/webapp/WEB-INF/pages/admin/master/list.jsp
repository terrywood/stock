...
<div class="listview">
    <table id="list_table" width="60%">
        ...
        <c:forEach var="obj" items="${pageList.data}" varStatus="vs">
            <tr>
                <td  rowspan="2" align="center" valign="middle">${obj.code}<br/>${obj.name}</td>
                <td>${obj.question}</td>
                <td><fmt:formatDate value="${obj.questionDate}" pattern="yyyy-MM-dd hh:mm"/> </td>
           </tr>
            <tr>
                <td>${obj.answer}</td>
                <td><fmt:formatDate value="${obj.answerDate}" pattern="yyyy-MM-dd hh:mm"/> </td>
           </tr>
        </c:forEach>
        </tbody>
    </table>
